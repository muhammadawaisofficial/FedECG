...
for strategy in ["FedAvg", "FedProx"]:
    ...
    for round_idx in range(1, NUM_ROUNDS + 1):
        ...
        # 2. Aggregation
        new_global = global_model.state_dict()
        for key in new_global.keys():
            orig_dtype = new_global[key].dtype
...
# Append the FedBN result we already got from previous script (Weighted F1: ~0.8688)
results.append({"Strategy": "FedBN", "Weighted_F1": 0.8688})

df = pd.DataFrame(results)
out_csv = os.path.join(RESULTS_DIR, "table10_strategy_comparison.csv")
